<template>
  <div class="tool">
    ...
    <a href="https://github.com/AxisZql/axis-chat-server" target="_blank" class="tool-github icon"><a-icon type="github"/></a>
    ...
    <a-modal title="用户信息" :visible="showUserModal" footer="" @cancel="showUserModal = false">
      <div class="tool-user">
        ...
        <div class="tool-user-info">
          <div class="tool-user-title">旧密码</div>
          <a-input-password class="tool-user-input" v-model="oldPassword" placeholder="请输入密码"></a-input-password>
          <!-- <a-button type="primary" @click="changePassword">确认</a-button> -->
        </div>
        <div class="tool-user-info">
          <div class="tool-user-title">新密码</div>
          <a-input-password class="tool-user-input" v-model="newPassword1" placeholder="请输入密码"></a-input-password>
          <!-- <a-button type="primary" @click="changePassword">确认</a-button> -->
        </div>
        <div class="tool-user-info">
          <div class="tool-user-title">确认密码</div>
          <a-input-password class="tool-user-input" v-model="newPassword2" placeholder="请输入密码"></a-input-password>
          <a-button type="primary" @click="changePassword">确认</a-button>
        </div>
      </div>
    </a-modal>
...
export default class GenalTool extends Vue {
  ...
  @appModule.Action('updateUser') updateUser: Function;
  ...
  async changeUserName() {
    ...
    if (data) {
      ...
      console.log(u);
      this.setUser(u);
      this.setUserGather(u);
      // todo:需要利用消息队列通知，通知其他用户个人信息改变
    }
  }

  async changePassword() {
    if (!passwordVerify(this.oldPassword)) {
      return;
    }
    let user: User = JSON.parse(JSON.stringify(this.user));
    // TODO: 修改了密码更新的接口
    let updatePwdReq: UpdateUserPasswordReq = {
      accessToken: this.token,
      oldPassword: this.oldPassword,
      newPassword1: this.newPassword1,
      newPassword2: this.newPassword2,
    };
    let res = await apis.patchPassword(updatePwdReq);
    let data = processReturn(res);
    if (data) {
      console.log('密码修改成功');
    }
  ...
  async handleUpload() {
    ...
    formData.append('avatarPic', this.avatar);

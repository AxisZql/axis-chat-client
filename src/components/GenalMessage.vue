  ...
  <div class="message">
    <div class="message-header">
      <div class="message-header-box">
        ...
        <genal-active v-if="groupGather['group_' + activeRoom.groupId]" type="group"></genal-active>
    ...
    <div class="message-main" :style="{ opacity: messageOpacity }">
      <div class="message-content">
        ...
        <template v-for="item in activeRoom.messages">
          <!-- 将消息列表中的消息进行展示 -->
          <div class="message-content-message" v-if="item" :key="item.snowId" :class="{ 'text-right': item.userid === user.userid }">
            <genal-avatar :data="item"></genal-avatar>
            <div>
              <a
                class="message-content-text"
                v-if="_isUrl(item.content) && item.messageType !== 'image'"
                :href="item.content"
                target="_blank"
                >{{ item.content }}</a
              >
...
export default class GenalMessage extends Vue {
  ...
  get chatName() {
    if (this.groupGather['group_' + this.activeRoom.groupId]) {
      return this.groupGather['group_' + this.activeRoom.groupId].groupName;
    } else {
      return this.userGather['user_' + this.activeRoom.userid].username;
    }
  }
  ...
  async getGroupMessages() {
    ...
    if (data) {
      ...
      for (let user of data.userArr) {
        if (!this.userGather['user_' + user.id]) {
          user.userid = user.id;
